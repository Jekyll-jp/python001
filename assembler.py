...
lines = []  # contains the lines of the file. ファイルの行を入れる
tokens = []  # contains all tokens of the source code.ソースコードのすべてのトークンが含まれています
...
# status register　登録情報
zeroFlag = False

# stack data structure　　構造
# push --> append
# pop --> pop
stack = []　
...
#     for i in range(9):
#         register.append(0)
#この関数はファイルをロードし、その行を読み取ります。
#global変数＝どの範囲からでもアクセスできる変数

#region loadFile(fileName)

def loadFile(fileName):
    """
        loadFile: This function loads the file and reads its lines.
        loadFile：この関数はファイルをロードし、その行を読み取ります。
    """
    global lines
    fo = open(fileName)　#中身を取得し１行づつlinesに入れる
    for line in fo:
        lines.append(line)
    fo.close()
...
#assert文　assert 条件式, 条件式がFalseの場合に出力するメッセージ
#
#region scan()
def scan():
    """
...
def main():
    ...
    #argvリストの２番目からfor
    #sys.argsのindex[0]には何かが入っているが使えない index[1]から使用ファイルpathが入っている
    for arg in sys.argv[1:]:

        resetInterpreter()  # resets interpreter mind　グローバル変数の初期化
